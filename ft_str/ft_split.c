...
/*   Updated: 2026/03/11 13:21:21 by babyf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	my_free(char **str, size_t i)
{
	while (i > 0)
	{
		free(str[--i]);
...
static char	*skip_char(char *s, char c)
{
	while (*s == c && *s != '\0')
		s++;
	return (s);
...
static char	*extract_substring(char *s, char c)
{
	char	*start;
	char	*end;
	char	*substring;
